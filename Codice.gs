...
function doGet(e) {
  try {
    ...
    if (mode === 'formazioni') return json_({status:'success', team:team, lineups:readLineup_(team, e.parameter.matchId || '')});
...
function getGames_(team) {
  ...
  const rows=[],manualRows=[]; values.slice(1).forEach((r,index)=>{if(!r[1]||!r[2]||!r[3])return;rows.push(r);if(!r[0]){r[0]=Utilities.getUuid();sheet.getRange(index+2,1).setValue(r[0]);manualRows.push(r);}});
...
function saveLineup_(team,data) {
  ...
  const lineup=normalizeLineup_(team,data.lineup||{}), matchId=String(data.matchId||'').replace(/[^a-zA-Z0-9-]/g,'').slice(0,50), name='Formazioni_'+team+(matchId?'_'+matchId:'');
...
function readLineup_(team,matchId) {
  const empty=normalizeLineup_(team,{}), name='Formazioni_'+team+(String(matchId||'').replace(/[^a-zA-Z0-9-]/g,'').slice(0,50)?'_'+String(matchId).replace(/[^a-zA-Z0-9-]/g,'').slice(0,50):''), sheet=SpreadsheetApp.getActive().getSheetByName(name); if(!sheet)return empty;
...
function writeBench_(sheet,row,col,players){sheet.getRange(row,col,1,7).merge().setValue('Panchina: '+(players.length?players.join(', '):'—')).setFontSize(9).setBackground('#f8fafc');}
...
function formatCalendar_(sheet){const rows=sheet.getLastRow(),cols=8,colors={Campionato:'#6f52b5',Amichevole:'#b14c7e',Torneo:'#ffc000'};sheet.getRange(1,1,1,cols).setValues([['ID','Squadra','Tipo','Data','Ora','Avversario','Campo','Mister']]).setBackground('#1e1b4b').setFontColor('#ffffff').setFontWeight('bold').setHorizontalAlignment('center');sheet.setFrozenRows(1);sheet.hideColumns(1);[110,110,105,90,80,190,90,220].forEach((w,i)=>sheet.setColumnWidth(i+1,w));if(rows<2)return;const data=sheet.getRange(2,1,rows-1,cols).getValues().filter(r=>r[1]&&r[3]);data.sort((a,b)=>String(a[1]).localeCompare(String(b[1]))||eventRank_(b[2])-eventRank_(a[2])||parseDate_(a[3])-parseDate_(b[3]));sheet.getRange(2,1,data.length,cols).setValues(data);sheet.getRange(2,4,data.length,1).setNumberFormat('dd/MM/yyyy');data.forEach((r,i)=>{const color=colors[r[2]]||'#ffffff';sheet.getRange(i+2,2,1,cols-1).setBackground(color).setFontColor(r[2]==='Torneo'?'#1f2937':'#ffffff');});}
