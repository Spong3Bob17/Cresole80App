...
function saveGameResult_(team,data) {
  ...
  if(!id) throw new Error('Gara non valida');
  const ids=sheet.getRange(2,1,Math.max(0,sheet.getLastRow()-1),1).getValues(); let row=0;
  for(let i=0;i<ids.length;i++)if(String(ids[i][0])===id){row=i+2;break;}
  if(!row||String(sheet.getRange(row,2).getValue())!==team) throw new Error('Gara non trovata');
